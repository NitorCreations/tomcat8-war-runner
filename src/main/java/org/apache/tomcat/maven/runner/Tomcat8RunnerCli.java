 ...
 * <ul>
 * <li>set default value for "env" system property
 * </ul>
 * 
 * @author Olivier Lamy
 * @since 2.0
...
public class Tomcat8RunnerCli {
  ...
  static Option sysProps = OptionBuilder.withDescription("use value for given property").hasArgs().withDescription("key=value").withValueSeparator()
      .create('D');
  ...
  static Option sessionManagerFactoryClassName = OptionBuilder.withArgName("className").hasArg()
      .withDescription("classname of a factory that creates a session manager").create("sessionManagerFactory");

  static Option loggerName = OptionBuilder.withArgName("loggerName").hasArg().withDescription("logger to use: slf4j to use slf4j bridge on top of jul")
      .create("loggerName");
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (line.hasOption(sessionManagerFactoryClassName.getOpt())) {
      tomcat8Runner.sessionManagerFactoryClassName = line.getOptionValue(sessionManagerFactoryClassName.getOpt());
    }
